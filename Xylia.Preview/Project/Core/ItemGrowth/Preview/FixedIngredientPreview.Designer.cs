
namespace Xylia.Preview.Project.Core.ItemGrowth.Preview
{
	partial class FixedIngredientPreview
	{
		/// <summary> 
		/// 必需的设计器变量。
		/// </summary>
		private System.ComponentModel.IContainer components = null;

		/// <summary> 
		/// 清理所有正在使用的资源。
		/// </summary>
		/// <param name="disposing">如果应释放托管资源，为 true；否则为 false。</param>
		protected override void Dispose(bool disposing)
		{
			if (disposing && (components != null))
			{
				components.Dispose();
			}
			base.Dispose(disposing);
		}

		#region 组件设计器生成的代码

		/// <summary> 
		/// 设计器支持所需的方法 - 不要修改
		/// 使用代码编辑器修改此方法的内容。
		/// </summary>
		private void InitializeComponent()
		{
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(FixedIngredientPreview));
			this.label2 = new System.Windows.Forms.Label();
			this.itemIconCell8 = new Xylia.Preview.Project.Core.Item.Cell.Basic.ItemIconCell();
			this.itemIconCell7 = new Xylia.Preview.Project.Core.Item.Cell.Basic.ItemIconCell();
			this.itemIconCell6 = new Xylia.Preview.Project.Core.Item.Cell.Basic.ItemIconCell();
			this.itemIconCell5 = new Xylia.Preview.Project.Core.Item.Cell.Basic.ItemIconCell();
			this.itemIconCell4 = new Xylia.Preview.Project.Core.Item.Cell.Basic.ItemIconCell();
			this.itemIconCell3 = new Xylia.Preview.Project.Core.Item.Cell.Basic.ItemIconCell();
			this.itemIconCell2 = new Xylia.Preview.Project.Core.Item.Cell.Basic.ItemIconCell();
			this.itemIconCell1 = new Xylia.Preview.Project.Core.Item.Cell.Basic.ItemIconCell();
			((System.ComponentModel.ISupportInitialize)(this.itemIconCell8)).BeginInit();
			((System.ComponentModel.ISupportInitialize)(this.itemIconCell7)).BeginInit();
			((System.ComponentModel.ISupportInitialize)(this.itemIconCell6)).BeginInit();
			((System.ComponentModel.ISupportInitialize)(this.itemIconCell5)).BeginInit();
			((System.ComponentModel.ISupportInitialize)(this.itemIconCell4)).BeginInit();
			((System.ComponentModel.ISupportInitialize)(this.itemIconCell3)).BeginInit();
			((System.ComponentModel.ISupportInitialize)(this.itemIconCell2)).BeginInit();
			((System.ComponentModel.ISupportInitialize)(this.itemIconCell1)).BeginInit();
			this.SuspendLayout();
			// 
			// label2
			// 
			this.label2.AutoSize = true;
			this.label2.BackColor = System.Drawing.Color.Transparent;
			this.label2.Font = new System.Drawing.Font("Microsoft YaHei UI", 12F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(134)));
			this.label2.ForeColor = System.Drawing.Color.White;
			this.label2.Location = new System.Drawing.Point(20, 78);
			this.label2.Name = "label2";
			this.label2.Size = new System.Drawing.Size(42, 21);
			this.label2.TabIndex = 4;
			this.label2.Text = "材料";
			// 
			// itemIconCell8
			// 
			this.itemIconCell8.BackColor = System.Drawing.Color.Transparent;
			this.itemIconCell8.ExtraBottomLeft = null;
			this.itemIconCell8.ExtraBottomRight = null;
			this.itemIconCell8.ExtraTopLeft = null;
			this.itemIconCell8.ExtraTopRight = null;
			this.itemIconCell8.ForeColor = System.Drawing.Color.Black;
			this.itemIconCell8.FrameImage = null;
			this.itemIconCell8.ItemIcon = ((System.Drawing.Bitmap)(resources.GetObject("itemIconCell8.ItemIcon")));
			this.itemIconCell8.Location = new System.Drawing.Point(437, 69);
			this.itemIconCell8.Name = "itemIconCell8";
			this.itemIconCell8.Scale = 42;
			this.itemIconCell8.ShowStackCount = true;
			this.itemIconCell8.ShowStackCountOnlyOne = true;
			this.itemIconCell8.SizeMode = System.Windows.Forms.PictureBoxSizeMode.Zoom;
			this.itemIconCell8.StackCount = ((uint)(1u));
			this.itemIconCell8.TabIndex = 12;
			this.itemIconCell8.TabStop = false;
			this.itemIconCell8.Visible = false;
			// 
			// itemIconCell7
			// 
			this.itemIconCell7.BackColor = System.Drawing.Color.Transparent;
			this.itemIconCell7.ExtraBottomLeft = null;
			this.itemIconCell7.ExtraBottomRight = null;
			this.itemIconCell7.ExtraTopLeft = null;
			this.itemIconCell7.ExtraTopRight = null;
			this.itemIconCell7.ForeColor = System.Drawing.Color.Black;
			this.itemIconCell7.FrameImage = null;
			this.itemIconCell7.ItemIcon = ((System.Drawing.Bitmap)(resources.GetObject("itemIconCell7.ItemIcon")));
			this.itemIconCell7.Location = new System.Drawing.Point(389, 69);
			this.itemIconCell7.Name = "itemIconCell7";
			this.itemIconCell7.Scale = 42;
			this.itemIconCell7.ShowStackCount = true;
			this.itemIconCell7.ShowStackCountOnlyOne = true;
			this.itemIconCell7.SizeMode = System.Windows.Forms.PictureBoxSizeMode.Zoom;
			this.itemIconCell7.StackCount = ((uint)(1u));
			this.itemIconCell7.TabIndex = 11;
			this.itemIconCell7.TabStop = false;
			this.itemIconCell7.Visible = false;
			// 
			// itemIconCell6
			// 
			this.itemIconCell6.BackColor = System.Drawing.Color.Transparent;
			this.itemIconCell6.ExtraBottomLeft = null;
			this.itemIconCell6.ExtraBottomRight = null;
			this.itemIconCell6.ExtraTopLeft = null;
			this.itemIconCell6.ExtraTopRight = null;
			this.itemIconCell6.ForeColor = System.Drawing.Color.Black;
			this.itemIconCell6.FrameImage = null;
			this.itemIconCell6.ItemIcon = ((System.Drawing.Bitmap)(resources.GetObject("itemIconCell6.ItemIcon")));
			this.itemIconCell6.Location = new System.Drawing.Point(341, 69);
			this.itemIconCell6.Name = "itemIconCell6";
			this.itemIconCell6.Scale = 42;
			this.itemIconCell6.ShowStackCount = true;
			this.itemIconCell6.ShowStackCountOnlyOne = true;
			this.itemIconCell6.SizeMode = System.Windows.Forms.PictureBoxSizeMode.Zoom;
			this.itemIconCell6.StackCount = ((uint)(1u));
			this.itemIconCell6.TabIndex = 10;
			this.itemIconCell6.TabStop = false;
			this.itemIconCell6.Visible = false;
			// 
			// itemIconCell5
			// 
			this.itemIconCell5.BackColor = System.Drawing.Color.Transparent;
			this.itemIconCell5.ExtraBottomLeft = null;
			this.itemIconCell5.ExtraBottomRight = null;
			this.itemIconCell5.ExtraTopLeft = null;
			this.itemIconCell5.ExtraTopRight = null;
			this.itemIconCell5.ForeColor = System.Drawing.Color.Black;
			this.itemIconCell5.FrameImage = null;
			this.itemIconCell5.ItemIcon = ((System.Drawing.Bitmap)(resources.GetObject("itemIconCell5.ItemIcon")));
			this.itemIconCell5.Location = new System.Drawing.Point(293, 69);
			this.itemIconCell5.Name = "itemIconCell5";
			this.itemIconCell5.Scale = 42;
			this.itemIconCell5.ShowStackCount = true;
			this.itemIconCell5.ShowStackCountOnlyOne = true;
			this.itemIconCell5.SizeMode = System.Windows.Forms.PictureBoxSizeMode.Zoom;
			this.itemIconCell5.StackCount = ((uint)(1u));
			this.itemIconCell5.TabIndex = 9;
			this.itemIconCell5.TabStop = false;
			this.itemIconCell5.Visible = false;
			// 
			// itemIconCell4
			// 
			this.itemIconCell4.BackColor = System.Drawing.Color.Transparent;
			this.itemIconCell4.ExtraBottomLeft = null;
			this.itemIconCell4.ExtraBottomRight = null;
			this.itemIconCell4.ExtraTopLeft = null;
			this.itemIconCell4.ExtraTopRight = null;
			this.itemIconCell4.ForeColor = System.Drawing.Color.Black;
			this.itemIconCell4.FrameImage = null;
			this.itemIconCell4.ItemIcon = ((System.Drawing.Bitmap)(resources.GetObject("itemIconCell4.ItemIcon")));
			this.itemIconCell4.Location = new System.Drawing.Point(245, 69);
			this.itemIconCell4.Name = "itemIconCell4";
			this.itemIconCell4.Scale = 42;
			this.itemIconCell4.ShowStackCount = true;
			this.itemIconCell4.ShowStackCountOnlyOne = true;
			this.itemIconCell4.SizeMode = System.Windows.Forms.PictureBoxSizeMode.Zoom;
			this.itemIconCell4.StackCount = ((uint)(1u));
			this.itemIconCell4.TabIndex = 8;
			this.itemIconCell4.TabStop = false;
			this.itemIconCell4.Visible = false;
			// 
			// itemIconCell3
			// 
			this.itemIconCell3.BackColor = System.Drawing.Color.Transparent;
			this.itemIconCell3.ExtraBottomLeft = null;
			this.itemIconCell3.ExtraBottomRight = null;
			this.itemIconCell3.ExtraTopLeft = null;
			this.itemIconCell3.ExtraTopRight = null;
			this.itemIconCell3.ForeColor = System.Drawing.Color.Black;
			this.itemIconCell3.FrameImage = null;
			this.itemIconCell3.ItemIcon = ((System.Drawing.Bitmap)(resources.GetObject("itemIconCell3.ItemIcon")));
			this.itemIconCell3.Location = new System.Drawing.Point(197, 69);
			this.itemIconCell3.Name = "itemIconCell3";
			this.itemIconCell3.Scale = 42;
			this.itemIconCell3.ShowStackCount = true;
			this.itemIconCell3.ShowStackCountOnlyOne = true;
			this.itemIconCell3.SizeMode = System.Windows.Forms.PictureBoxSizeMode.Zoom;
			this.itemIconCell3.StackCount = ((uint)(1u));
			this.itemIconCell3.TabIndex = 7;
			this.itemIconCell3.TabStop = false;
			this.itemIconCell3.Visible = false;
			// 
			// itemIconCell2
			// 
			this.itemIconCell2.BackColor = System.Drawing.Color.Transparent;
			this.itemIconCell2.ExtraBottomLeft = null;
			this.itemIconCell2.ExtraBottomRight = null;
			this.itemIconCell2.ExtraTopLeft = null;
			this.itemIconCell2.ExtraTopRight = null;
			this.itemIconCell2.ForeColor = System.Drawing.Color.Black;
			this.itemIconCell2.FrameImage = null;
			this.itemIconCell2.ItemIcon = ((System.Drawing.Bitmap)(resources.GetObject("itemIconCell2.ItemIcon")));
			this.itemIconCell2.Location = new System.Drawing.Point(149, 69);
			this.itemIconCell2.Name = "itemIconCell2";
			this.itemIconCell2.Scale = 42;
			this.itemIconCell2.ShowStackCount = true;
			this.itemIconCell2.ShowStackCountOnlyOne = true;
			this.itemIconCell2.SizeMode = System.Windows.Forms.PictureBoxSizeMode.Zoom;
			this.itemIconCell2.StackCount = ((uint)(1u));
			this.itemIconCell2.TabIndex = 6;
			this.itemIconCell2.TabStop = false;
			this.itemIconCell2.Visible = false;
			// 
			// itemIconCell1
			// 
			this.itemIconCell1.BackColor = System.Drawing.Color.Transparent;
			this.itemIconCell1.ExtraBottomLeft = null;
			this.itemIconCell1.ExtraBottomRight = null;
			this.itemIconCell1.ExtraTopLeft = null;
			this.itemIconCell1.ExtraTopRight = null;
			this.itemIconCell1.ForeColor = System.Drawing.Color.Black;
			this.itemIconCell1.FrameImage = null;
			this.itemIconCell1.ItemIcon = ((System.Drawing.Bitmap)(resources.GetObject("itemIconCell1.ItemIcon")));
			this.itemIconCell1.Location = new System.Drawing.Point(101, 69);
			this.itemIconCell1.Name = "itemIconCell1";
			this.itemIconCell1.Scale = 42;
			this.itemIconCell1.ShowStackCount = true;
			this.itemIconCell1.ShowStackCountOnlyOne = true;
			this.itemIconCell1.SizeMode = System.Windows.Forms.PictureBoxSizeMode.Zoom;
			this.itemIconCell1.StackCount = ((uint)(1u));
			this.itemIconCell1.TabIndex = 5;
			this.itemIconCell1.TabStop = false;
			this.itemIconCell1.Visible = false;
			// 
			// FixedIngredientPreview
			// 
			this.AutoScaleDimensions = new System.Drawing.SizeF(6F, 12F);
			this.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Font;
			this.BackColor = System.Drawing.Color.Black;
			this.BackgroundImageLayout = System.Windows.Forms.ImageLayout.Center;
			this.Controls.Add(this.itemIconCell8);
			this.Controls.Add(this.itemIconCell7);
			this.Controls.Add(this.itemIconCell6);
			this.Controls.Add(this.itemIconCell5);
			this.Controls.Add(this.itemIconCell4);
			this.Controls.Add(this.itemIconCell3);
			this.Controls.Add(this.itemIconCell2);
			this.Controls.Add(this.itemIconCell1);
			this.Controls.Add(this.label2);
			this.DoubleBuffered = true;
			this.Name = "FixedIngredientPreview";
			this.Size = new System.Drawing.Size(619, 197);
			((System.ComponentModel.ISupportInitialize)(this.itemIconCell8)).EndInit();
			((System.ComponentModel.ISupportInitialize)(this.itemIconCell7)).EndInit();
			((System.ComponentModel.ISupportInitialize)(this.itemIconCell6)).EndInit();
			((System.ComponentModel.ISupportInitialize)(this.itemIconCell5)).EndInit();
			((System.ComponentModel.ISupportInitialize)(this.itemIconCell4)).EndInit();
			((System.ComponentModel.ISupportInitialize)(this.itemIconCell3)).EndInit();
			((System.ComponentModel.ISupportInitialize)(this.itemIconCell2)).EndInit();
			((System.ComponentModel.ISupportInitialize)(this.itemIconCell1)).EndInit();
			this.ResumeLayout(false);
			this.PerformLayout();

		}

		#endregion

		private System.Windows.Forms.Label label2;
		private Item.Cell.Basic.ItemIconCell itemIconCell1;
		private Item.Cell.Basic.ItemIconCell itemIconCell2;
		private Item.Cell.Basic.ItemIconCell itemIconCell3;
		private Item.Cell.Basic.ItemIconCell itemIconCell4;
		private Item.Cell.Basic.ItemIconCell itemIconCell5;
		private Item.Cell.Basic.ItemIconCell itemIconCell6;
		private Item.Cell.Basic.ItemIconCell itemIconCell7;
		private Item.Cell.Basic.ItemIconCell itemIconCell8;
	}
}
