...
namespace Xylia.Match.Windows.Panel
{
	public partial class MatchProp : UserControl
	{
		...
		private void Online_Searcher_BtnClick(object sender, EventArgs e)
		{
			...
			if (!string.IsNullOrEmpty(Select.SelectedName)) Xylia.Configure.Ini.WriteValue(Util.Static.Enums.Section.Match, "Chv_Name", Chv_Path.Text = Select.SelectedName);
			if (!string.IsNullOrEmpty(Select.SelectedPath)) Xylia.Configure.Ini.WriteValue(Util.Static.Enums.Section.Match, "Chv_Path", Select.SelectedPath);
		}
